...
我做的是把前后端分离, 用k8s和docker来把所有的feature模块分成micro service, 用一个golang写的gateway负责把http转换成grpc,然后feature service和gateway之间用grpc来通信. 前端用react 和react native来写web和app.数据库从sqllite换成了postgresql. 用户注册和验证用aws cognito, 基本上是一个比较典型的micro service架构,在prototype p0阶段比较适合几个人用适合自己pace的方式快速build一个系统,

这其实是一个比较简单的典型前后端分离架构,用编译语言golang来写一个相对solid的关键节点,剩下的就糙快猛的搭起来. 这个系统里之前的llm部分是用ReAct实现,我本想在这一部分中心设计一下,但是因为时间没有继续. 
